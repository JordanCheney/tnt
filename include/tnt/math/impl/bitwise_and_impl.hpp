#ifndef TNT_MATH_BITWISE_AND_IMPL_HPP
#define TNT_MATH_BITWISE_AND_IMPL_HPP
...
struct OptimizedBitwiseAnd<LeftType, RightType>
{
    static void eval(Tensor<LeftType>& tensor, const RightType& _scalar)
    {
        ...
        for ( ; num_blocks--; offset += OptimalSIMDSize<LeftType>::value) {
            ...
            auto result = simdpp::bit_and(block, scalar_vec);
    ...
    static void eval(Tensor<LeftType>& left, const Tensor<RightType>& right)
    {
        ...
        for ( ; num_blocks--; offset += OptimalSIMDSize<LeftType>::value) {
            ...
            auto result = simdpp::bit_and(l_block, r_block);
...
TEST_CASE_TEMPLATE("bitwise_and(Tensor<unsigned>&, Scalar)", T, test_unsigned_data_types)
{
    ...
    auto test_shape = [](const Shape& shape) {
        ...
        REQUIRE(((TensorType(shape, 1)   & 2)  == TensorType(shape, 0)));
        REQUIRE(((TensorType(shape, 1)   & 3)  == TensorType(shape, 1)));
        REQUIRE(((TensorType(shape, 170) & 85) == TensorType(shape, 0)));
    };
...
TEST_CASE_TEMPLATE("bitwise_and(Tensor<signed>&, Scalar)", T, test_signed_data_types)
{
    using TensorType = Tensor<T>;
    ...
    auto test_shape = [](const Shape& shape) {
        REQUIRE(((TensorType(shape, 0)   & 0)   == TensorType(shape, 0)));
        REQUIRE(((TensorType(shape, 1)   & 1)   == TensorType(shape, 1)));
        REQUIRE(((TensorType(shape, -1)  & 2)   == TensorType(shape, 2)));
        REQUIRE(((TensorType(shape, 1)   & 3)   == TensorType(shape, 1)));
        REQUIRE(((TensorType(shape, -86) & 85)  == TensorType(shape, 0)));
        REQUIRE(((TensorType(shape, -8)  & -16) == TensorType(shape, -16)));
    };
...
#endif // TNT_MATH_BITWISE_AND_IMPL_HPP
