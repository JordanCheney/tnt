#ifndef TNT_MATH_BITWISE_OR_HPP
#define TNT_MATH_BITWISE_OR_HPP
...
template <
          ...
          typename DeviceType
         >
inline void bitwise_or(Tensor<LeftType, DeviceType>& left, const RightType& scalar)
{
    static_assert(std::is_integral<LeftType>::value,
                  "Bitwise or is only meaningful for integer types");

    detail::OptimizedBitwiseOr<LeftType, RightType, DeviceType>().run(left, scalar);
}
...
template <
          ...
          typename DeviceType
         >
inline void bitwise_or(Tensor<LeftType, DeviceType>& left, const Tensor<RightType, DeviceType>& right)
{
    static_assert(std::is_integral<LeftType>::value,
                  "Bitwise or is only meaningful for integer types");
    ...
    detail::OptimizedBitwiseOr<LeftType, RightType, DeviceType>().run(left, right);
}

} // namespace tnt

#endif // TNT_MATH_BITWISE_OR_HPP
